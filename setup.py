...
ladon_command = 'ladon-%d.%d-ctl%s' % (sys.version_info.major,
                                       sys.version_info.minor, '.py' if sys.platform.find('win') == 0 else '')
scriptfiles = [os.path.join('scripts', ladon_command),
               os.path.join('scripts', 'ladon-ctl')]
...
setup(
    ...
    author='Anuj Singh',
    author_email='anujsingh@mastersindia.co',
    maintainer='Anuj Singh',
    maintainer_email='anujsingh@mastersindia.co',
    url='https://github.com/masters-india/ladon',
        install_requires=['jinja2', 'sphinx',
                          'sphinx_bootstrap_theme', 'chardet',
                          'python_memcached','redis'],
        requires=['jinja2', 'sphinx', 'sphinx_bootstrap_theme',
                  'chardet', 'python_memcached','redis'],
    provides=['ladon'],
    license='LGPL3',
    scripts=scriptfiles,
